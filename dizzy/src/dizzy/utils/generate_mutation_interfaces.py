...
This utility reads a mutations.yaml file and generates Protocol classes that define
...
def generate_mutation_interfaces(
    mutations_yaml: Path,
    output_file: Path,
    gen_import_path: str = 'gen.mutations'
):
    """
    Generate Protocol interface file from mutations schema.

    Args:
        mutations_yaml: Path to mutations.yaml schema file
    ...
    lines = [
        ...
        f'Generated from {mutations_yaml.name} by dizzy.utils.generate_mutation_interfaces',
        '"""',
        '',
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python -m dizzy.utils.generate_mutation_interfaces <mutations.yaml> <output.py> [import_path]")
        sys.exit(1)

    mutations_yaml = Path(sys.argv[1])
    output_file = Path(sys.argv[2])
    gen_import_path = sys.argv[3] if len(sys.argv) > 3 else 'gen.mutations'

    generate_mutation_interfaces(mutations_yaml, output_file, gen_import_path)
